...
$stmt = $db->prepare("SELECT p.*, c.text AS changelog_text, d.text AS description_text, a.* FROM portableapps p LEFT JOIN changelogs c ON p.changelog_id = c.id LEFT JOIN descriptions d ON p.description_id = d.id JOIN archives a ON a.papp_id = p.id");
$stmt->execute();
